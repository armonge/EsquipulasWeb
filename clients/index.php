...
try{
    ...
    SELECT
        ...
        SUM(IF(rec.iddocumento IS NOT NULL, rec.total , 0) )
            + SUM(IF(ret.iddocumento IS NOT NULL, ret.total , 0) ) totalrecibos
    FROM personas p
    ...
    LEFT JOIN docpadrehijos dph ON rec.iddocumento = dph.idpadre
    LEFT JOIN documentos ret ON ret.iddocumento = dph.idhijo AND ret.idtipodoc = {$docids["RETENCION"]} AND rec.idestado = {$docstates["CONFIRMADO"]}
    WHERE p.tipopersona = {$persontypes["CLIENTE"]}
    GROUP BY p.idpersona
    ORDER BY activo DESC
    ";
    $rsClients = $dbc->query($query);
